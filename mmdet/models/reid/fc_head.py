...
class FcHead(BaseModule):
    ...
    def __init__(self,
        ...
        super(FcHead, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, feats: Tuple[torch.Tensor], data_samples):
        """The forward process."""
        # Multiple stage inputs are acceptable
        # but only the last stage will be used.
        feats_last = feats[-1]
        feats_last = self.avgpool(feats_last)
        feats_last = torch.flatten(feats_last, 1)  # batch * dim

        for m in self.fcs:
            feats_last = m(feats_last)
        feats_last = self.fc_out(feats_last)

        loss = self.loss_by_feat(feats_last, data_samples)
    ...
    def loss(self, feats: Tuple[torch.Tensor],
        ...
        feats = self(feats) # 这里会重复进入到forward函数中
    ...
    def loss_by_feat(self, feats: torch.Tensor,
        ...
        gt_label = torch.tensor([i.metainfo['class_id'] - 1 for i in data_samples]).to(feats.device)

        if self.loss_cls:
            feats_ln = self.relu(self.ln(feats))
            cls_score = self.classifier(feats_ln)
            losses['ce_loss'] = self.loss_cls(cls_score, gt_label)

            acc = torch.sum(torch.argmax(cls_score, dim=1) == gt_label) / len(gt_label)
            losses.update(
                {f'accuracy_top-{self.topk}': acc})
    ...
    def predict(
        ...
        feats_ln = self.relu(self.ln(feats_last))
        ...
        for score in cls_score:
            pre = list()
            pre_s = list()
            score_sort_index = torch.argsort(score).flip(0)
            score_sort = torch.sort(score, descending=True)
            for indexes in score_sort_index:
                i = int(indexes.item())
                pre.append(i)

            for scores in score_sort[0]:
                s = float(scores.item())
                pre_s.append(s)
        ...
        #pre_classes = torch.tensor(pre_classes)
        #pre_classes = torch.argmax(cls_score, dim=1)
        # The part can not be traced by torch.fx
        #data_samples = self.predict_by_feat(feats, data_samples)

        return pre_classes,pre_scores
